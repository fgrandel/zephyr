...
augmented with information from bindings (see bindings.py) and include some
...
The top-level entry point for the library is the EDT class. See its constructor
...
from bindings import (
    ...
    Property,
    str_as_token,
    _BindingLoader,  # TODO: Remove dependency
)
...
class EDTProperty(Property[PropertyValType, "EDTNode"]):
    """
    devicetree-specific specialization of a Property.

    val:
      The value of the type-to-python mapping for devicetree sources is as
      follows:
    ...
    See the docstring of the Property base class for all other class members.
    """
    pass
...
class EDTNode:
    ...
    These attributes are available on EDTNode objects:
    ...
    parent:
      The EDTNode instance for the devicetree parent of the EDTNode, or None if the
      node is the root node

    children:
      A dictionary with the EDTNode instances for the devicetree children of the
      node, indexed by name

    dep_ordinal:
      A non-negative integer value such that the value for a EDTNode is
    ...
    def __init__(
        ...
        self.props: Dict[str, EDTProperty] = {}
    ...
    @property
    def parent(self) -> Optional["EDTNode"]:
        "See the class docstring"
        return self.edt._node2enode.get(self._node.parent) # type: ignore

    @property
    def children(self) -> Dict[str, "EDTNode"]:
        "See the class docstring"
    ...
    @property
    def required_by(self) -> List["EDTNode"]:
        "See the class docstring"
        return self.edt._graph.required_by(self)

    @property
    def depends_on(self) -> List["EDTNode"]:
        "See the class docstring"
    ...
    @property
    def flash_controller(self) -> "EDTNode":
        "See the class docstring"
    ...
    def __repr__(self) -> str:
        if self.binding_paths:
            ...
            binding = "no binding"
        return f"<EDTNode {self.path} in '{self.edt.dts_path}', {binding}>"

    def _init_bindings(self) -> None:
        # Initializes EDTNode.matching_compat, EDTNode._binding, and
        # EDTNode.binding_path.
        #
        # EDTNode._binding holds the data from the node's binding file, in the
    ...
    def _init_props(self, default_prop_types: bool = False,
        ...
        if self._prop2specs:
            ...
            for name in self._node.props:
                ...
                self.props[name] = EDTProperty(prop_spec, val, self)
    ...
    def _named_cells(
        self, controller: "EDTNode", data: bytes, basename: str
    ) -> Dict[str, int]:
...
class EDT:
    ...
    nodes:
      A list of EDTNode objects for the nodes that appear in the devicetree

    compat2nodes:
      A collections.defaultdict that maps each 'compatible' string that appears
      on some EDTNode to a list of Nodes with that compatible.
    ...
    compat2vendor:
      A collections.defaultdict that maps each 'compatible' string that appears
      on some EDTNode to a vendor name parsed from vendor_prefixes.

    compat2model:
      A collections.defaultdict that maps each 'compatible' string that appears
      on some EDTNode to a model name parsed from that compatible.
    ...
    chosen_nodes:
      ...
      and values are converted to EDTNode objects. Note that properties of the
      /chosen node which can't be converted to a EDTNode are not included in
      the value.
    ...
    scc_order:
      ...
      on any EDTNode in a subsequent list. Each list defines a Strongly
    ...
    def __init__(self,
        ...
        self.nodes: List[EDTNode] = []
        self.compat2nodes: Dict[str, List[EDTNode]] = defaultdict(list)
        self.compat2okay: Dict[str, List[EDTNode]] = defaultdict(list)
        self.compat2notokay: Dict[str, List[EDTNode]] = defaultdict(list)
        self.compat2vendor: Dict[str, str] = defaultdict(str)
        self.compat2model: Dict[str, str]  = defaultdict(str)
        self.label2node: Dict[str, EDTNode] = {}
        self.dep_ord2node: Dict[int, EDTNode] = {}
        ...
        self._node2enode: Dict[dtlib_Node, EDTNode] = {}
    ...
    def get_node(self, path: str) -> EDTNode:
        """
    ...
    @property
    def chosen_nodes(self) -> Dict[str, EDTNode]:
        ret: Dict[str, EDTNode] = {}
    ...
    def chosen_node(self, name: str) -> Optional[EDTNode]:
        """
        Returns the EDTNode pointed at by the property named 'name' in /chosen, or
    ...
    @property
    def scc_order(self) -> List[List[EDTNode]]:
        try:
            return self._graph.scc_order()
        except Exception as e:
            raise EDTError(e)

    def _process_properties_r(self, root_node: EDTNode, props_node: EDTNode):
        """
        ...
        """
        # An EDTNode depends on any Nodes present in 'phandle',
        ...
        # An EDTNode depends on whatever supports the interrupts it
        # generates.
    ...
    def _process_properties(self, node: EDTNode) -> None:
        """
    ...
    def _init_graph(self) -> None:
        # Constructs a graph of dependencies between EDTNode instances,
        ...
        for node in self.nodes:
            ...
            # An EDTNode always depends on its parent.
    ...
    def _init_nodes(self) -> None:
        ...
        for dt_node in self._dt.node_iter():
            ...
            node = EDTNode(dt_node, self, self._fixed_partitions_no_bus)
            self.nodes.append(node)
            self._node2enode[dt_node] = node

        for node in self.nodes:
            # Initialize properties that may depend on other EDTNode objects having
