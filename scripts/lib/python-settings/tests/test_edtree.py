...
def test_warnings(caplog):
    ...
    with from_here():
        STree("test-bindings").add_partial_tree("dts", EDTree("test.dts")).process()
...
def test_hierarchy():
    ...
    assert (
        ...
        == "{'child-1': <STMergedNode /parent/child-1 in 'test.dts', no binding>, 'child-2': <STMergedNode /parent/child-2 in 'test.dts', no binding>}"
    )
...
def test_binding_inference():
    ...
    with from_here():
        stree: STree = (
            STree("test-bindings")
            .add_partial_tree("dts", EDTree("test.dts", warnings))
            .process()
        )

    assert str(stree.node_by_path("/zephyr,user").props) == "{}"

    with from_here():
        stree = (
            STree("test-bindings", warnings)
            .add_partial_tree(
                "dts",
                EDTree(
                    "test.dts",
    ...
    verify_phandle_array_prop(
        zephyr_user,
        "phandle-array-foos",
        [(stree.node_by_path("/ctrl-2"), {"one": 1, "two": 2})],
    )
...
def test_multi_bindings():
    ...
    with from_here():
        stree: STree = (
            STree(["test-bindings", "test-bindings-2"])
            .add_partial_tree("dts", EDTree("test-multidir.dts"))
            .process()
        )
...
def test_deepcopy():
    with from_here():
        ...
        stree_copy: STree = deepcopy(stree)
    ...
    assert stree_copy.bindings_dirs == ["test-bindings", "test-bindings-2"]
    assert stree_copy.bindings_dirs is not stree.bindings_dirs
    ...
    test_equal_but_not_same("compat2notokay", equal_key2paths)
    ...
    test_equal_but_not_same("_compats")
    ...
    test_equal_but_not_same("_path2node", equal_key2path)

    edtree: EDTree = stree.partial_tree_by_source_id("dts")
    edtree_copy: EDTree = stree_copy.partial_tree_by_source_id("dts")
...
def verify_error(dts, dts_file, expected_err, expected_msg):
    ...
    with pytest.raises(expected_err) as e:
        STree([]).add_partial_tree("dts", EDTree(dts_file)).process()
