...
from bindings import STBinding, bindings_from_paths
...
Compat2Binding = Dict[Compat, STBinding]
Binding2Changes = Dict[STBinding, List[BindingChange]]
...
def get_binding_changes(
    binding_start: STBinding, binding_end: STBinding
) -> List[BindingChange]:
...
def get_modified_property_type(
    binding_start: STBinding, binding_end: STBinding, common_props: Set[str]
) -> List[BindingChange]:
    ...
    with tempfile.TemporaryDirectory(prefix='dt_bindings_worktree') as tmpdir:
        with git_worktree(tmpdir, commit):
            ...
            binding_files.extend(glob.glob(f'{tmpdir_bindings}/**/*.yaml',
                                           recursive=True))
            bindings: List[STBinding] = bindings_from_paths(
...
def compatible_sort_key(data: Union[Compat, STBinding]) -> str:
...
class Printer:
    ...
    def print_compat2binding(
        self, compat2binding: Compat2Binding, formatter: Callable[[STBinding], str]
    ) -> None:
