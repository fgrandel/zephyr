...
static int flash_mock_init(const struct device *dev)
{
	bool is_provisioned, is_reserved_memory;

	ARG_UNUSED(dev);

	is_provisioned = IS_ENABLED(CONFIG_FLASH_SIMULATOR_PROVISION);
	is_reserved_memory = DT_NODE_HAS_PROP(DT_PARENT(SOC_NV_FLASH_NODE), memory_region);

	if (!is_reserved_memory || (is_provisioned && FLASH_SIMULATOR_ERASE_VALUE != 0x00)) {
