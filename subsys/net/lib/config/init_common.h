...
#define NET_CONFIG_NET_DEVICE(iface_node_id) CFG_POINTER(iface_node_id, device)
...
#define NET_CONFIG_SUBNODE(_node_id, _subnode) CFG_CHILD(_node_id, _subnode)

#define NET_CONFIG_SUBNODE2(_node_id, _subnode, _subnode2)                                         \
	CFG_CHILD(DT_CHILD(_node_id, _subnode), _subnode2)

#define NET_CONFIG_IF_SUBSTATE(_node_id, _subnode, _bracketed_then)                                \
	IF_ENABLED(CFG_NODE_IS_ENABLED(NET_CONFIG_SUBNODE(_node_id, _subnode)),                    \
		   _bracketed_then)

#define NET_CONFIG_COND_SUBSTATE(_node_id, _subnode, _bracketed_then, _bracketed_else)             \
	COND_CODE_1(CFG_NODE_IS_ENABLED(NET_CONFIG_SUBNODE(_node_id, _subnode)),                   \
...
#define NET_CONFIG_SUBSTATE_STR_ARRAY(_iface_node_id, _subnode, _str_array_field)                  \
	static const char *NET_CONFIG_SUBSTATE2_NAME(_iface_node_id, _subnode,                     \
						     _str_array_field)[] =                         \
		CFG_PROP(NET_CONFIG_SUBNODE(_iface_node_id, _subnode), _str_array_field)
