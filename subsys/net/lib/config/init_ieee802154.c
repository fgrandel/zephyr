...
#define NET_CONFIG_IEEE802154_RADIO_DRIVER_STATE(_drv_node_id)                                     \
	{.dev = DEVICE_DT_GET(_drv_node_id),                                                       \
	 .pan_id = CFG_PROP(_drv_node_id, pan_id),                                                 \
	 .ext_addr = CFG_PROP(_drv_node_id, ext_addr),                                             \
	 .default_channel = CFG_PROP(_drv_node_id, default_channel),                               \
	 .default_tx_pwr = CFG_PROP(_drv_node_id, default_tx_pwr),                                 \
	 .ack_required = !CFG_PROP(_drv_node_id, no_ack),                                          \
	 IF_ENABLED(CONFIG_NET_L2_IEEE802154_SECURITY,                                             \
		    (.sec_key = CFG_PROP_OR(_drv_node_id, sec_key, NULL),                          \
		     .sec_level = CFG_PROP(_drv_node_id, sec_level),                               \
		     .sec_key_mode = CFG_PROP(_drv_node_id, sec_key_mode),))},

#define NET_CONFIG_IEEE802154_RADIO_DRIVER_CONFIG_VERIFY(_drv_node_id)                             \
	IF_ENABLED(CONFIG_NET_L2_IEEE802154_SECURITY,                                              \
		   (BUILD_ASSERT(CFG_NODE_HAS_PROP(_drv_node_id, sec_key) &&                       \
					 sizeof(CFG_PROP_OR(_drv_node_id, sec_key, "")) == 16,     \
				 "A 16 byte security key is required when "                        \
				 "CONFIG_NET_L2_IEEE802154_SECURITY = y.");))

CFG_FOREACH_ENABLED(zephyr_net_l2_ieee802154_native,
...
static struct net_config_ieee802154_radio_driver net_config_ieee802154_radio_drivers[] = {
	 ...
	 *		CFG_FOREACH_ENABLED(zephyr_net_l2_ieee802154,
	 *				    NET_CONFIG_IEEE802154_RADIO_DRIVER_STATE)
	 */
};
