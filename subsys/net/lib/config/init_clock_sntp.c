...
static int sntp_init_helper(struct sntp_time *tm, const char *server, int timeout)
{
	...
	if (!net_ipv4_is_addr_unspecified(&iface->config.dhcpv4.ntp_addr)) {
		...
		return sntp_simple_addr((struct sockaddr *)&sntp_addr, sizeof(sntp_addr), timeout,
					tm);
	}
...
int net_init_clock_via_sntp(struct net_if *iface, const char *server, int timeout)
{
	struct sntp_time ts;
