...
#define NET_CONFIG_SNTP_SERVER_STATE(_sntp_server_node_id)                                         \
	{                                                                                          \
		.server_name = CFG_PROP(_sntp_server_node_id, server_name),                        \
		.timeout = CFG_PROP(_sntp_server_node_id, timeout),                                \
	}

#define NET_CONFIG_SNTP_SERVER(_sntp_server_node_id)                                               \
	static struct net_config_sntp_server CONCAT(sntp_server_, _sntp_server_node_id) =          \
		NET_CONFIG_SNTP_SERVER_STATE(_sntp_server_node_id)

#define NET_CONFIG_SNTP_SERVERS(_sntp_servers_node_id)                                             \
	CFG_FOREACH_CHILD_ENABLED(_sntp_servers_node_id, NET_CONFIG_SNTP_SERVER);

#define NET_CONFIG_SNTP_SERVER_PTR_OR_NULL(_iface_node_id)                                         \
	COND_CODE_1(                                                                               \
		CFG_NODE_HAS_PROP(_iface_node_id, sntp_server),                                    \
		(&CONCAT(sntp_server_, CFG_POINTER(_iface_node_id, sntp_server))),                 \
		(NULL))

#define NET_CONFIG_IPV6_PREFIX_STATE(_ipv6_prefix_node_id)                                         \
	{                                                                                          \
		.addr = CFG_PROP(_ipv6_prefix_node_id, addr),                                      \
		.len = CFG_PROP(_ipv6_prefix_node_id, len),                                        \
		.lifetime = CFG_PROP(_ipv6_prefix_node_id, lifetime),                              \
	},
...
#define NET_CONFIG_IPV6_PREFIXES_STATE(_iface_node_id, _ipv6_prefixes_node_id,                     \
	...
	(static struct net_config_ipv6_prefix NET_CONFIG_SUBSTATE2_NAME(_iface_node_id, ipv6,      \
									prefixes)[] = {            \
		 CFG_FOREACH_CHILD_ENABLED(_ipv6_prefixes_node_id,                                 \
...
#define NET_CONFIG_NUM_IPV6_PREFIXES(_iface_node_id)                                               \
	NET_CONFIG_COND_SUBSTATE(                                                                  \
		...
		(CFG_CHILD_NUM_ENABLED(NET_CONFIG_SUBNODE2(_iface_node_id, ipv6, prefixes))), (0))
...
#define NET_CONFIG_IPV6_DHCP_CLIENT_STATE(_ipv6_dhcp_client_node_id, _ipv6_dhcp_client_state_name) \
	(static struct net_config_ipv6_dhcp_client _ipv6_dhcp_client_state_name = {                \
		 .req_addr = CFG_PROP(_ipv6_dhcp_client_node_id, req_addr),                        \
		 .req_prefix = CFG_PROP(_ipv6_dhcp_client_node_id, req_prefix),                    \
	 };)
...
#define NET_CONFIG_IPV6_STATE(_iface_node_id, _ipv6_node_id, _ipv6_state_name)                     \
	...
	static struct net_config_ipv6 _ipv6_state_name = {                                        \
		 ...
		 .hop_limit = CFG_PROP(_ipv6_node_id, hop_limit),                                  \
		 .mcast_hop_limit = CFG_PROP(_ipv6_node_id, mcast_hop_limit),                      \
		 .num_addrs = CFG_PROP_LEN(_ipv6_node_id, addrs),                                  \
		 .num_mcast_addrs = CFG_PROP_LEN(_ipv6_node_id, mcast_addrs),                      \
...
#define NET_CONFIG_IPV4_STATE(_iface_node_id, _ipv4_node_id, _ipv4_state_name)                     \
	...
	static struct net_config_ipv4 _ipv4_state_name = {                                        \
		 ...
		 .gateway = CFG_PROP(_ipv4_node_id, gateway),                                      \
		 .ttl = CFG_PROP(_ipv4_node_id, ttl),                                              \
		 .mcast_ttl = CFG_PROP(_ipv4_node_id, mcast_ttl),                                  \
		 .dhcp_client = NET_CONFIG_COND_SUBSTATE(_ipv4_node_id, dhcp_client, (1), (0)),    \
		 .dhcp_server_base_addr = CFG_PROP_OR(                                             \
			 NET_CONFIG_SUBNODE(_ipv4_node_id, dhcp_server), base_addr, NULL),         \
		 .autoconf = CFG_PROP(_ipv4_node_id, autoconf),                                    \
		 .num_addrs = CFG_PROP_LEN(_ipv4_node_id, addrs),                                  \
		 .num_mcast_addrs = CFG_PROP_LEN(_ipv4_node_id, mcast_addrs),                      \
	};)

#define NET_CONFIG_VLAN_STATE(_iface_node_id, _vlan_node_id, _vlan_state_name)                     \
	(static struct net_config_vlan _vlan_state_name = {                                        \
		 .tag = CFG_PROP(_vlan_node_id, tag),                                              \
	};)
...
#define NET_CONFIG_IFACE_STATE(_iface_node_id)                                                     \
	{                                                                                          \
		...
		.set_iface_name = CFG_PROP_OR(_iface_node_id, set_iface_name, NULL),               \
		...
		.set_flags = CFG_PROP(_iface_node_id, set_flags),                                  \
		.clear_flags = CFG_PROP(_iface_node_id, clear_flags),                              \
		.is_default = CFG_PROP(_iface_node_id, is_default),                                \
	},

BUILD_ASSERT(CFG_NUM_INST_ENABLED(zephyr_net_lib_sntp_servers) <= 1,
	     "Only one SNTP servers configuration with status 'okay' is supported.");

CFG_FOREACH_ENABLED(zephyr_net_lib_sntp_servers, NET_CONFIG_SNTP_SERVERS)
CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_IPV6_PREFIXES)
CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_IPV6_DHCP_CLIENT)
CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_IPV6)
CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_IPV4)
CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_VLAN)
...
static struct net_config_iface net_config_ifaces[] = {
	CFG_FOREACH_ENABLED(zephyr_net_iface, NET_CONFIG_IFACE_STATE)};
